...
export default {
      ...
      contentDiv.innerHTML = '<div style="display: grid; gap: 20px;">' +
        '<div style="padding: 20px; background: white; border-radius: 8px; border: 1px solid #e2e8f0;">' +
          ...
          '<div style="display: grid; gap: 15px;">' +
            ...
            '</div>' +
            '<div>' +
              '<label style="display: block; margin-bottom: 8px; color: #64748b; font-size: 0.9rem;">评论模板 *</label>' +
              '<textarea id="activity-template" placeholder="例如：我是{nickname}，我支持这个活动！" style="width: 100%; padding: 8px; border: 1px solid #e2e8f0; border-radius: 6px; height: 100px; resize: vertical;"></textarea>' +
              '<p style="margin: 8px 0 0 0; font-size: 0.85rem; color: #64748b;">支持变量：{time} {nickname} {timeto=YYYY-MM-DD}</p>' +
            '</div>' +
            '<div>' +
  ...
  window.loadActivities = async function() {
    ...
    try {
      ...
      listDiv.innerHTML = activities.map(function(a) {
        return '<div style="padding: 15px; border: 1px solid #e2e8f0; border-radius: 8px; margin-bottom: 15px; background: #f8fafc;">' +
          ...
          '<div style="font-size: 0.85rem; color: #94a3b8; margin-top: 8px;">模板：' + a.template.substring(0, 50) + (a.template.length > 50 ? '...' : '') + '</div>' +
        '</div>';
      }).join('');
    } catch (err) {
      listDiv.innerHTML = '<div style="color: #ef4444;">加载失败</div>';
