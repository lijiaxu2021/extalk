...
- **🚨 用户举报系统** - 每条评论可举报，hCaptcha 防止滥用
- **🤖 自动隐藏机制** - 举报次数达到阈值自动隐藏（默认 3 次）
- **📧 邮件实时通知** - 被举报后立即通知管理员
- **⚡ 快速处理** - 管理员可删除/隐藏/恢复/忽略举报
...
2. 完成 hCaptcha 人机验证
3. 举报成功，计数 +1
4. 达到阈值（默认 3 次）自动隐藏评论
5. 管理员收到邮件通知
6. 管理员可在后台删除/隐藏/恢复/忽略举报

### Q7: 如何调整自动隐藏阈值？

**A:** 在数据库中修改用户的 `auto_hide_threshold` 字段：

```sql
UPDATE users SET auto_hide_threshold = 5 WHERE role = 'admin';
```
...
- ✅ 评论滑出滑入动画
- ✅ 完整举报系统（hCaptcha 验证 + 自动隐藏 + 邮件通知）
- ✅ 管理员评论管理后台（删除/隐藏/恢复/忽略举报）
